...
##==========================
                len_s = mask_para[1].to(device)
                len_t = mask_para[2].to(device)

                pair_logits = self.models.pair_head(feature_s, feature_t, len_s, len_t)  # B×1
                pair_prob = torch.sigmoid(pair_logits).mean().item()
                self.writer.add_scalar('pair_head/train_prob', pair_prob, i)
##==========================
                '''matching loss'''
                loss_fn = FocalLoss()
...
##========================
                len_s = mask_para[1].to(device)
                len_t = mask_para[2].to(device)
                pair_logits = self.models.pair_head(feature_s, feature_t, len_s, len_t)
                pair_prob = torch.sigmoid(pair_logits).mean().item()
                self.writer.add_scalar('pair_head/val_prob', pair_prob, i)
##========================
                '''matching loss'''
                loss_fn = FocalLoss()
                pad_mask = torch.add(pad_mask, gt_mask)  # padded mask with gt label.
...
if __name__ == "__main__":
    ...
    if opt.model_type == 'matching_train': 
        ...
        ST2_searcher.searching_start()
'''python PairingNet_train_val_test.py --model_type matching_train --train_set "D:\jigsaw\china2\Data Generation Code\ori_train_set.pkl"  --valid_set "D:\jigsaw\china2\Data Generation Code\ori_valid_set.pkl"  --test_set  "D:\jigsaw\china2\Data Generation Code\ori_test_set.pkl" --matching_batch_size 2 --patch_size 5 --max_length 1600'''